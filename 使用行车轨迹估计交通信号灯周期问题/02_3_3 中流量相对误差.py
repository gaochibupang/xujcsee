...
# 读取csv文件
address = r'E:/数据采集与预处理/数据采集与预处理/12_pandas案例/第二次/代码/v_middle.csv'
v_middle = pd.read_csv(address, encoding='gbk')

# 获取所有独特的车辆 ID
ID = v_middle['vehicle_id'].unique()
...
for current_vehicle in ID:
    vehicle_data = v_middle[v_middle['vehicle_id'] == current_vehicle]
...
print('\n中流量数据红灯时间的相对误差为：', relative_error)
