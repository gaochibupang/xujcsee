...
# 筛选出数值型列，并排除不需要处理的列
numeric_cols = data.columns.drop(exclude_cols)
# 初始化异常值计数Series
...
# 使用列表推导式和集合来去除重复项
seen = set()
unique_most_important_features = [x for x in most_important_features_list if not (x in seen or seen.add(x))]
