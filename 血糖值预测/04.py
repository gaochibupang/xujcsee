...
address_test = r'E:/数据采集与预处理/数据采集与预处理/12_pandas案例/D/无血糖值的检测数据.csv'
data_test = pd.read_csv(address, encoding='gbk')
'''数据清洗及预处理'''
# 查看变量
...
data_test['性别'] = label_encoder.fit_transform(data_test['性别'])
...
data_test['体检日期'] = pd.to_datetime(data_test['体检日期'], format='%d/%m/%Y', errors='coerce')
data_test['体检日期'] = (data_test['体检日期'] - reference_date).dt.days
...
print("1、缺失值处理")
# 查看缺失值比例
print("查看训练集缺失值比例", data.isnull().sum()/len(data))
print("查看测试集缺失值比例", data_test.isnull().sum()/len(data_test))
# 对每列使用中位数填充缺失值
...
# 训练集
numeric_cols_test = data_test.columns
for col in numeric_cols_test:
    data_test[col] = data_test[col].fillna(data_test[col].median())

# 再次查看缺失值
print("再次查看训练集缺失值数量:", data.isnull().sum())
print("再次查看测试集缺失值数量:", data_test.isnull().sum())
...
# 初始化异常值计数Series
outlier_counts_before = pd.Series(index=numeric_cols, dtype=float)
outlier_counts_after = pd.Series(index=numeric_cols, dtype=float)
outlier_counts_before_test = pd.Series(index=numeric_cols_test, dtype=float)
outlier_counts_after_test = pd.Series(index=numeric_cols_test, dtype=float)

# 处理异常值
def handle_outliers(data, cols, counts_before, counts_after):
    for col in cols:
        ...
        counts_before[col] = outliers.sum()
        ...
        counts_after[col] = outliers_after.sum()

# 处理训练集
handle_outliers(data, numeric_cols, outlier_counts_before, outlier_counts_after)

# 处理测试集
handle_outliers(data_test, numeric_cols_test, outlier_counts_before_test, outlier_counts_after_test)

# 打印原始和处理后的异常值计数
print("训练集原始异常值计数:", outlier_counts_before)
print("训练集处理后异常值计数:", outlier_counts_after)
print("测试集原始异常值计数:", outlier_counts_before_test)
print("测试集处理后异常值计数:", outlier_counts_after_test)
...
data_test = data_test.drop(['id', '体检日期'], axis=1)

# 由于乙肝数据存在大量缺失值，考虑到实际性，在此也做剔除处理
data = data.drop(['乙肝e抗体', '乙肝e抗原', '乙肝核心抗体', '乙肝表面抗体', '乙肝表面抗原'], axis=1)
data_test = data_test.drop(['乙肝e抗体', '乙肝e抗原', '乙肝核心抗体', '乙肝表面抗体', '乙肝表面抗原'], axis=1)
...
data.to_csv('xiangguanxingfenxi.csv', index=False)

# ----------------------------------- 主成分分析 ---------------------------------------------
# print(data.columns.tolist())
# 1、去除'血糖'列，因为我们只对其他变量进行PCA
...
for feature in unique_most_important_features:
    print(feature)  # 血红蛋白、总胆固醇、红细胞平均血红蛋白量、高密度脂蛋白胆固醇、年龄、中性粒细胞%、嗜碱细胞%、尿素、*碱性磷酸酶

# 10、使用Pandas选择这些特征
X_new = X[unique_most_important_features]
test_new = data_test[unique_most_important_features]

# 生成新的数据集merged_df，保存到CSV
merged_df = pd.concat([X_new, y], axis=1)
merged_df.to_csv('merged_data.csv', index=False)
test_new.to_csv('test_new.csv', index=False)

print("**********************************   第三题   **************************************************")
'''思路：首先建立随机森林模型，通过参数网络找到最佳参数，然后交叉验证评估模型，最后预测以及可视化。'''

# 读取csv文件
address = r'E:/数据采集与预处理/数据采集与预处理/12_pandas案例/merged_data.csv'
...
# 1、模型训练与参数优化
# 定义参数网格
param_grid = {'n_estimators': [20, 50, 70, 80],
...
grid_search = GridSearchCV(estimator=rf_model, param_grid=param_grid, cv=5, scoring='neg_mean_squared_error', verbose=2,
                           n_jobs=-1)  # 通过交叉验证（cv=5）来评估不同参数组合的性能

# 使用训练数据拟合GridSearchCV对象，从而找到最佳参数组合
grid_search.fit(X, y)
...
# 绘制折线图
plt.figure(figsize=(10, 6))
plt.plot(range(N), y_sample, label='Actual Values', color='blue', marker='o')
plt.plot(range(N), predictions_sample, label='Predicted Values', color='orange', marker='o')
plt.xlabel('Sample Index')
plt.ylabel('Blood Glucose Level')
plt.title('Comparison of Actual and Predicted Blood Glucose Levels (Sampled Data)')
plt.legend()
plt.grid(True)
plt.show()
...
# 绘制预测值与实际值的对比散点图
plt.figure(figsize=(10, 6))
plt.scatter(y, predictions, color='blue', label='Predicted vs Actual', s=5)
plt.plot([y.min(), y.max()], [y.min(), y.max()], 'k--', lw=4, label='Perfect Prediction')
...
# 读取待预测数据处理后的csv文件
address = r'E:/数据采集与预处理/数据采集与预处理/12_pandas案例/test_new.csv'
test_new = pd.read_csv(address, encoding='utf-8')

# 使用最佳模型进行预测
predictions_test = best_model.predict(test_new)

# 创建一个新的DataFrame来存储预测结果
predicted_df = pd.DataFrame(predictions_test, index=test_new.index, columns=['血糖'])

# 将预测结果添加到 new_data_test 中（作为新列）
test_new.loc[:, '血糖'] = predictions_test

# 将包含预测结果的DataFrame保存为CSV文件
test_new.to_csv('血糖预测值.csv')

# 查看“血糖”分布情况，绘制分布直方图
plt.figure(figsize=(10, 6))  # 设置图形的大小
plt.hist(data['血糖'], bins=10, color='yellow', alpha=0.7)  # bins参数控制直方图的柱子数量
plt.rcParams['font.sans-serif'] = ['SimHei']  # 设置字体为黑体
plt.title('预测血糖分布直方图')  # 设置图形的标题
plt.xlabel('血糖值')  # 设置x轴标签
plt.ylabel('频数')  # 设置y轴标签
plt.grid(axis='y', alpha=0.75)  # 显示y轴的网格线，并设置透明度
# 以6.7为分界添加一条虚线
plt.axvline(x=6.7, color='red', linestyle='--')  # 使用红色虚线表示
plt.show()
